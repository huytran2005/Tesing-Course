...
def _stub_redis(monkeypatch):
    ...
    from controllers import menu_item_controller
    from controllers import restaurant_controller

    monkeypatch.setattr(menu_item_controller, "redis_client", _RedisStub(), raising=False)
    monkeypatch.setattr(restaurant_controller, "redis_client", _RedisStub(), raising=False)
    yield


@pytest.fixture(autouse=True)
def _stub_report_overview(monkeypatch):
    # report_controller.build_overview uses Postgres-specific SQL functions;
    # stub cached_overview so /reports/overview can be tested with SQLite.
    from controllers import report_controller

    def _fake_cached_overview(restaurant_id, five_min_key):
        return {
            "chart": [],
            "summary": {
                "total_revenue": 0.0,
    ...
    monkeypatch.setattr(report_controller, "cached_overview", _fake_cached_overview)
    yield
